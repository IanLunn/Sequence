...
If desired, Sequence's canvas and/or content can be animated.

[Animating the canvas](#animating-the-canvas) consists of laying out Sequence's steps using HTML data-attributes to position them in 2D or 3D space. With the steps laid out in this way, each time a step is navigated to, Sequence will automatically animate its canvas accordingly so the current step always appears within the container.

[Animating content](#animating-content) is achieved via the CSS classes `animate-in` and `animate-out`. When a step is navigated to, it is given the `animate-in` class, allowing you to specify animations on the content via CSS. Similarly, a step that is navigated away from, is given a class of `animate-out`.

On top of canvas and content animation, the Sequence element is also given indexed class names `step1`, `step2`, and so on -- the index number representing the ID of the current step. These class names can be used to apply additional styles to Sequence where necessary.

#### Animating The Canvas

To animate the canvas, steps should be laid out using the following HTML data attributes:
...
<div id="sequence">
  <ul class="sequence-canvas">
    <li data-sequence-x="100" data-sequence-rotate="45">Step 1</li>
  </ul>
</div>
```

The above example will move the first step 100px on the X axis and rotate it clockwise by 45 degrees. When Sequence navigates to this step, the canvas element `<ul class="sequence-canvas">` will be given the CSS property: `transform: translateX(-100px) rotate(-45deg)` to make the step appear perfectly within the container boundaries.

##### `data-sequence-x`

Translates the step by the given amount of pixels along the X axis (in 2D space, up-and-down).

Equivalent of CSS `transform: translateX()`.

##### `data-sequence-y`

Translates the step by the given amount of pixels along the Y axis (in 2D space, side-to-side).

Equivalent of CSS `transform: translateY()`.

##### `data-sequence-z`

Translates the step by the given amount of pixels along the Z axis (in 3D space, to-and-from).

Equivalent of CSS `transform: translateZ()`.

##### `data-sequence-rotate`

Rotates the step clockwise around its origin (as specified by the `transform-origin` property applied to the step) by the specified angle in degrees (in 2D space).

Equivalent of CSS `transform: rotate()`.

##### `data-sequence-rotate-x`

Rotates the step clockwise around its origin (as specified by the `transform-origin` property applied to the step) by the specified angle in degrees (in 3D space).

Equivalent of CSS `transform: rotateX()`.

##### `data-sequence-rotate-y`

Rotates the step clockwise around its origin (as specified by the `transform-origin` property applied to the step) by the specified angle in degrees (in 3D space).

Equivalent of CSS `transform: rotateY()`.

##### `data-sequence-scale`

Specifies a 2D scaling operation. `data-sequence-scale="2"` will make a step twice as big for example.

Equivalent of CSS `transform: scale()`.
