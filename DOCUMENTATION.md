...
When the preloader is being used, the Sequence element is give a class of `preloading` during the preloading phase, then `preloaded` when preloading is complete.
...
The following CSS is added to layout the preloader and make the loading indicators animate:
...
.sequence-preloader .preload .circle:nth-child(2) {
  animation-delay: .15s;
}

.sequence-preloader .preload .circle:nth-child(3) {
  animation-delay: .3s;
}
